...
from jVMC.nets.sym_wrapper import avgFun_Coefficients_Exp, SymNet
from jVMC.nets.two_nets_wrapper import TwoNets
from jVMC.util.key_gen import generate_seed, format_key
from jVMC.sharding_config import MESH, DEVICE_SPEC, REPLICATED_SPEC, DEVICE_SHARDING, REPLICATED_SHARDING
...
class NQS:
    """
    Neural quantum state (NQS) ansatz for variational quantum algorithms.

    This class can operate in two modes:
        #. Single-network ansatz: Quantum state of the form :math:`\\psi_\\theta(s)\\equiv\\exp(r_\\theta(s))`, \
           where the network :math:`r_\\theta` is either:
        ...
        #. Two-network ansatz: Quantum state of the form \
           ...
           :math:`\\varphi_{\\theta_\\phi}` parametrized by real valued parameters :math:`\\theta_r,\\theta_\\phi`.

    Arguments:
        * ``net``: Variational network or tuple of networks. A network has to be registered as a pytree node and provide \
          a ``__call__`` function for evaluation. If a tuple of two networks is given, the first is used for the logarithmic \
          amplitude and the second for the phase of the wave function coefficient.
        * ``sampleShape``: Shape of configurations (sample space).
        * ``batchSize``: Batch size for batched network evaluation across all devices. Exactly one of ``batchSize`` or \
          ``batchSize_per_device`` must be specified. Choice of this parameter impacts performance: too small values are \
          limited by memory access overheads, too large values can lead to "out of memory" issues.
        * ``batchSize_per_device``: Batch size per device. Exactly one of ``batchSize`` or ``batchSize_per_device`` must be specified.
        * ``logarithmic``: Boolean variable indicating whether the network returns logarithmic \
          (:math:`\\log\\psi_\\theta(s)`) or plain (:math:`\\psi_\\theta(s)`) wave function coefficients. Default is ``True``.
        * ``seed``: Seed for the PRNG to initialize the network parameters. If ``None``, a random seed is generated.
        * ``orbit``: Orbit defining the symmetry operations (instance of ``util.symmetries.LatticeSymmetry``). \
          If provided, the wave function is symmetrized to be invariant under symmetry operations.
        * ``avgFun``: Reduction operation for the symmetrization. Default is ``avgFun_Coefficients_Exp``.
    """
