...
from jVMC_exp.symmetry_projector import (
    ProjectedOrbitNet,
    SymmetryProjector,
...
def test_translation_permutations_characters_and_fermion_signs():
    Lx, Ly = 3, 2
    q = (1, 1)
...
def test_snake_and_custom_site_enumeration_for_translation_and_validation():
    Lx, Ly = 4, 3
...
def test_d4_permutations_and_point_group_fallbacks():
...
def test_rectangle_d2_and_chain_reflection():
...
def test_unique_symmetry_projector_accumulates_duplicate_characters():
...
def test_spin_flip_and_boson_particle_hole_local_maps():
...
def test_spinful_fermion_spin_flip_local_map_and_doublon_sign():
...
def test_spinless_particle_hole_uses_canonical_fock_sign():
...
def test_plural_spinful_particle_type_and_projector_weight_alias():
...
def test_spinful_particle_hole_uses_canonical_fock_sign():
...
def test_spinful_particle_hole_matches_dense_fock_operator():
    n_sites = 2
...
def test_particle_hole_matrix_interchanges_creation_and_annihilation():
    n_modes = 4
...
def test_spinful_spin_flip_matches_dense_fock_operator():
    n_sites = 2
...
def test_composed_translation_particle_hole_states_and_signs():
    Lx, Ly = 2, 2
    state = np.array([3, 1, 2, 0], dtype=np.int32)
...
def test_projector_multiplication_ux():
...
def test_projected_orbit_net_average_modes():
...
def test_non_projector_averages_warn_for_nontrivial_weights():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
...
def test_exp_average_includes_nontrivial_momentum_weights():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
    symm = square_translation_symmetry(4, 1, "spin", q=(1, 0))
    base_net = _FirstSiteNet()
...
def test_projected_orbit_net_accepts_custom_average_function():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
    symm = square_translation_symmetry(4, 1, "spin", q=(1, 0))
    base_net = _FirstSiteNet()
...
def test_coefficient_state_net_is_vectorized_over_transformed_states():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
...
def test_projected_orbit_net_rejects_both_coefficient_interfaces():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
    symm = square_translation_symmetry(4, 1, "spin")
    projected = ProjectedOrbitNet(
        base_net=_FirstSiteNet(),
...
def test_exp_average_preserves_exact_projector_cancellation():
...
def test_autoregressive_sep_average_uses_fixed_half_log_prob_factor():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
    symm = spin_flip_symmetry(4, 1)
...
def test_autoregressive_projector_rejects_nontrivial_factors():
    state = jnp.asarray([0, 1, 1, 0], dtype=jnp.int32)
